...
class InvitationExpirationTests(TransactionTestCase):
    ...
    def test_invitation_acceptance_before_expiration(self):
        # Create a test user for the owner of the invitation
        test_user = models.User.objects.create_user(username='inviting_user_before', password='12345')
        accepting_user = models.User.objects.create_user(username='accepting_user_before', password='12345')
        future_date = timezone.now() + timedelta(days=7)  # Invitation expires in the future
        invitation = models.Invitation.objects.create(owner=test_user, is_open=False, expires_at=future_date)
        ...
        try:
            ...
            self.assertFalse(invitation.is_expired(), "The invitation should not be expired yet.")
